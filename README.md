...
Plus **full-agent golden tests** (`tests/test_golden_agent.py`) that ask the real agent a question in plain English (not hand-written SQL) and check the number in its written answer against the same official population figures. This is `test_golden_data.py`'s counterpart one level up the stack — it can catch a regression in how the model itself translates a question into a query, which a SQL-only test can't. Slower and uses real Anthropic + Snowflake calls, so kept to two cases and skipped (not failed) unless both `SNOWFLAKE_ACCOUNT` and `ANTHROPIC_API_KEY` are set.

**CI** (`.github/workflows/test.yml`) runs the mocked suite automatically on every push/PR to `main`. Both sets of golden tests are deliberately **not** wired into CI -- no Snowflake/Anthropic credentials are configured as GitHub secrets, so they show as skipped there, by design, for three reasons: (1) **cost** -- CI runs on every push, and burning real Snowflake compute against the account's spend cap on every single push (rather than real usage) is wasteful; (2) **security surface** -- it would mean another place the Snowflake password lives, for no real benefit; (3) **reliability** -- live-infrastructure tests are exposed to the same transient failures (e.g. warehouse cold-start) documented in `REFLECTION.md`, which would make CI flaky/untrustworthy rather than a fast, dependable signal. So the split is intentional: CI checks control-flow correctness on every push for free; the golden-data tests check actual data correctness, run manually, when it matters (e.g. before a release).
